...
public class ScaleModeTutor extends Tutor
{
    ...
    private Question getScaleMode() {
        ...
        String rootNoteName = "";
        final Integer scaleType = Tutor.randInt(0, 1);
        ...
        String degree = Integer.toString(Tutor.randInt(1, 7));
        rootNoteName = this.rootNotes.get(Tutor.randInt(0, this.rootNotes.size() - 1));
        ...
        final Integer correctAns = Tutor.randInt(0, 1);
        String answerString;
        if (correctAns == 1) {
            for (String previousDegree = degree; previousDegree.equals(degree); degree = Integer.toString(Tutor.randInt(1, 7))) {}
            answerString = "True";
        }
        else {
            for (String previousRootNoteName = rootNoteName; previousRootNoteName.equals(rootNoteName); rootNoteName = this.rootNotes.get(Tutor.randInt(0, this.rootNotes.size() - 1))) {}
            answerString = "False";
        }
        this.parser.executeCommand(String.format("modeOf(%s, \"%s\", %s)", rootNoteName, scaleTypeName, degree));
