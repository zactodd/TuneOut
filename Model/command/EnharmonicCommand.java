...
public class EnharmonicCommand extends Command
{
    ...
    public EnharmonicCommand(final List<String> arg) {
        ...
        if (arg.size() == 1) {
            final Note note = EnharmonicMap.getEquivalentEnharmonic(NoteMap.getNote(Command.defaultOctave(inputNote)));
            if (note != null) {
                this.returnValue = Command.translateOctave(inputNote, note.getNoteWithOctave());
            }
