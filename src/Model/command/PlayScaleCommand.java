...
public class PlayScaleCommand extends Command implements OnSuccess
{
    ...
    private String correctReturnValue(final String scaleName, final String input, final String output, final String orderType, final String swingType) {
        return String.format("%s scale %s played in %s order and %s style: ", scaleName, translate(input, output), orderType, swingType);
    }
    ...
    private String processScale() {
        this.playScale = true;
        String returnValue = this.correctReturnValue(this.scaleName, this.inputNote, defaultNote(this.inputNote), this.orderType, this.swingType);
        if (checkMidiInput(this.inputNote)) {
